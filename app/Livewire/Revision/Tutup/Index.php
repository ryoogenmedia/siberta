...
class Index extends Component
{
    ...
    public function rows(){
        $query = Berkas::where('status_file','revision')
            ...
            ->when($this->filters['tipe_dokumen'], function($query, $type){
                $query->where('category','tutup')->where('status_file', 'revision')->where('type_document', $type);
            })
            ->when($this->filters['status_file'], function($query, $status){
                $query->where('category','tutup')->where('status_file', 'revision')->where('status_file', $status);
            })
            ->when($this->filters['tanggal_awal'], function ($query, $tanggalAwal) {
                $query->where('category','tutup')->where('status_file', 'revision')->whereHas('revision', function($query) use ($tanggalAwal){
                    $query->where('date_revision', '>=', $tanggalAwal);
                });
            })
            ->when($this->filters['tanggal_akhir'], function ($query, $tanggalAkhir) {
                $query->where('category','tutup')->where('status_file', 'revision')->whereHas('revision', function($query) use ($tanggalAkhir){
                    $query->where('date_revision', '<=', $tanggalAkhir);
                });
            })
            ->when($this->filters['nama_file'], function($query, $namaFile){
                $query->where('category','tutup')->where('status_file', 'revision')->where('name_file', $namaFile);
            })
            ->when($this->filters['nim'], function($query, $nim){
                $query->where('category','tutup')->where('status_file', 'revision')->whereHas('mahasiswa', function($query) use ($nim){
                    $query->where('nim', $nim);
                });
            })
            ->when($this->filters['program_studi'], function($query, $programStudi){
                $query->where('category','tutup')->where('status_file', 'revision')->whereHas('mahasiswa', function($query) use ($programStudi){
                    $query->where('program_studi', $programStudi);
                });
            })
            ->when($this->filters['search'], function ($query, $search) {
                $query->where('category','tutup')->where('status_file', 'revision')->whereHas('mahasiswa', function($query) use ($search){
            ...
            })->where('category','tutup')->where('status_file', 'revision');
    ...
    {
        return view('livewire.revision.tutup.index');
    }
}
