...
class Index extends Component
{
    ...
    public function rows(){
        $query = Berkas::query()
            ...
            ->when($this->filters['tipe_dokumen'], function($query, $type){
                $query->where('category','hasil')->where('type_document', $type);
            })
            ->when($this->filters['status_file'], function($query, $status){
                $query->where('category','hasil')->where('status_file', $status);
            })
            ->when($this->filters['tanggal_awal'], function ($query, $tanggalAwal) {
                $query->where('category','hasil')->where('date_upload', '>=', $tanggalAwal);
            })
            ->when($this->filters['tanggal_akhir'], function ($query, $tanggalAkhir) {
                $query->where('category','hasil')->where('date_upload', '<=', $tanggalAkhir);
            })
            ->when($this->filters['nama_file'], function($query, $namaFile){
                $query->where('category','hasil')->where('name_file', $namaFile);
            })
            ->when($this->filters['nim'], function($query, $nim){
                $query->where('category','hasil')->whereHas('mahasiswa', function($query) use ($nim){
                    $query->where('nim', $nim);
                });
            })
            ->when($this->filters['program_studi'], function($query, $programStudi){
                $query->where('category','hasil')->whereHas('mahasiswa', function($query) use ($programStudi){
                    $query->where('program_studi', $programStudi);
                });
            })
            ->when($this->filters['search'], function ($query, $search) {
                $query->where('category','hasil')->whereHas('mahasiswa', function($query) use ($search){
    ...
    {
        return view('livewire.berkas.hasil.index');
    }
}
